  ...
  async displayRecipes (recipesData) {
    ...
    if (recipesData && recipesData.length) {
      ...
      for (let i = 0; i < recipesData.length; i++) {
        ...
        if (!recipesData[i].id && !recipesData[i].idMeal) return;
        
        let res = await fetch(`https://www.themealdb.com/api/json/v1/1/lookup.php?i=${recipesData[i].id | recipesData[i].idMeal}`);
        ...
        <div class="recipe-card bg-white rounded-xl overflow-hidden shadow-sm hover:shadow-lg transition-all cursor-pointer group" 
          ...
          <div class="relative h-48 overflow-hidden">
            <img class="w-full h-full object-cover group-hover:scale-110 transition-transform duration-500" src="${recipe.strMealThumb}" alt="${recipe.strMeal}" loading="lazy" />
          ...
          <div class="p-4">
            ...
            <p class="text-xs text-Base2 text-gray-600 mb-3 line-clamp-2">
              ${recipe.strInstructions || 'No instructions.'}
            </p>
            <div class="flex items-center justify-between text-xs">
              <span class="font-semibold Name text-gray-900">
                <i class="fa-solid fa-utensils text-emerald-600 mr-1"></i> ${recipe.strMeal}
              </span>
              <span class="font-semibold Area text-gray-500">
                <i class="fa-solid fa-globe text-blue-500 mr-1"></i> ${recipe.strArea}
              </span>
            </div>
          </div>
        </div>`;
